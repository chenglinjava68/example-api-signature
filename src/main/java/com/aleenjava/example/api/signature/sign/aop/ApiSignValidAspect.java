...
import com.aleenjava.example.api.signature.sign.core.ApiSignNonceStrCache;
import com.aleenjava.example.api.signature.sign.core.ApiSignProperties;
import com.aleenjava.example.api.signature.sign.core.ApiSignValidException;
import com.aleenjava.example.api.signature.sign.filter.ApiSignRequestWrapper;
import com.aleenjava.example.api.signature.sign.util.HttpRequestUtil;
...
public class ApiSignValidAspect {
     ...
     * 切入点
     */
    @Pointcut("@annotation(com.aleenjava.example.api.signature.sign.aop.ApiSignValid)")
    ...
    public void before(JoinPoint joinPoint) throws ApiSignValidException {
        try {
            ...
            if (nonceStrCache.isExist(app.getId(), nonce)) {
                throw new Exception();
            }
            // 缓存随机字符串
            nonceStrCache.put(app.getId(), nonce);

            // 签名校验
            boolean isSigned = verifySign(app, sign, timestamp, nonce, request);
            if (!isSigned) {
                throw new ApiSignValidException();
            }
    ...
    private boolean verifySign(ApiSignProperties.App app, String signStr, long timestamp, String nonce, HttpServletRequest request) {
        try {
            ...
            SortedMap<String, Object> allParams = HttpRequestUtil.getAllParams(requestWrapper);
            // 加入签名信息
